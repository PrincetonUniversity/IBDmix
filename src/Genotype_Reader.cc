...
Genotype_Reader::Genotype_Reader(std::istream * genotype, std::istream *mask,
    ...
    genotype(genotype), mask(mask), archaic_error(archaic_error),
...
void Genotype_Reader::process_line_buffer(bool selected) {
    ...
    for (int i = 0; i < sample_mapper.size(); i++) {
        lod_scores[i] = calculate_lod(
                buffer[sample_mapper.sample_to_index[i]*2]);
        recover_type[i] = 0;
    }
...
void Genotype_Reader::update_lod_cache(char archaic, double freq_b,
    ...
    double err_1 = (1 - archaic_error) * (1 - modern_error)
        + archaic_error * modern_error;
    double err_2 = (1 - archaic_error) * modern_error
        + archaic_error * (1 - modern_error);

    if (archaic == '0') {
        if (selected) {
            lod_cache[0] = log10(err_1 / freq_a / err_0);
            lod_cache[1] = log10(0.5 / err_0 *
                    (err_2 / freq_b + err_1 / freq_a));
        } else {
