...
void Sample_Mapper::find_archaic(const std::string &archaic) {
  if (archaic == "") {
    archaic_index = 0;
    return;
  }
  ...
  if (it == samples.end()) {
    throw std::invalid_argument("Unable to find archaic '" + archaic + '\'');
  }
...
void Sample_Mapper::map(std::vector<std::string> requested_samples) {
  ...
  if (requested_samples.empty()) {
    ...
    int index = 0;
    for (unsigned int i = 0; i < samples.size(); ++i, ++index) {
      if (index == archaic_index) ++index;
    ...
    for (std::string &sample : requested_samples) {
      ...
      if (it == samples.end()) {
        throw std::invalid_argument("Unable to find sample '" + sample + '\'');
      }
...
int Sample_Mapper::initialize(std::istream &genotype,
  ...
  if (std::getline(genotype, line)) {
    ...
    iss >> token;  // ref
    iss >> token;  // alt
    while (iss >> token) samples.emplace_back(token);
  }
  if (samples.empty())
    throw std::invalid_argument(
        "Unable to find samples. Invalid genotype file");

  // if samples is provided, populate requested samples
  std::vector<std::string> requested;
  while (requested_samples >> token) requested.emplace_back(token);
