...
double cal_pearsoncorr(int *x, int *y, int n){
	int num = 0;
...
int main(int argc, char *argv[]){
	...
	for(ulnt l = 0; l < Lnum; l++) ibd[l] = new ulnt[2];
	for(int a = 0; a < Anum; a++) lod[a] = new double[Lnum];
	...
	for(ulnt l = 0; l < Lnum; l++){
		...
		for(int a = 0; a < Anum; a++){
			...
			if(agt[l][a]==9) sel[l] = 0;
		}

		Inum = 0;
		for(int i = 0; i < Indnum; i++){
			fscanf(inp[0],"%d",&c);
			if(indsel[i]==1){mgt[l][Inum] = c; Inum++;}
		}
	}
	fclose(inp[0]);
