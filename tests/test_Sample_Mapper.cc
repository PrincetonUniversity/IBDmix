...
TEST(SampleMapper, CanMap) {
  ...
  for (int i = 0; i < 4; i++) ASSERT_EQ(i + 1, mapper.sample_to_index[i]);
  ...
  mapper.map(requested);
  for (int i = 0; i < 4; i++) ASSERT_EQ(i, mapper.sample_to_index[i]);
  ASSERT_EQ(mapper.sample_to_index.size(), 4);
  mapper.sample_to_index.clear();
  ...
  mapper.map(requested);
  for (int i = 0; i < 2; i++) ASSERT_EQ(i, mapper.sample_to_index[i]);
  for (int i = 2; i < 4; i++) ASSERT_EQ(i + 1, mapper.sample_to_index[i]);
  ASSERT_EQ(mapper.sample_to_index.size(), 4);
  mapper.sample_to_index.clear();

  mapper.samples =
      std::vector<std::string>{"n1", "s1", "s2", "s3", "s4", "s5", "s6", "s7"};
  requested = std::vector<std::string>{"s6", "n1", "s3"};
  mapper.map(requested);
  int result[] = {6, 0, 3};
  for (int i = 0; i < 3; i++) ASSERT_EQ(result[i], mapper.sample_to_index[i]);
  ASSERT_EQ(mapper.sample_to_index.size(), 3);
  mapper.sample_to_index.clear();

  // duplicate samples
  requested = std::vector<std::string>{"s1", "s1", "s1"};
  mapper.samples = std::vector<std::string>{"n1", "s1"};
  mapper.map(requested);
  int result2[] = {1, 1, 1};
  for (int i = 0; i < 3; i++) ASSERT_EQ(result2[i], mapper.sample_to_index[i]);
  ASSERT_EQ(mapper.sample_to_index.size(), 3);
  mapper.sample_to_index.clear();
